...
Real OAuth apps were never registered this pass (needs the human developer's own Google Cloud Console / Dropbox App Console / Azure AD accounts, per the blocking prerequisite above) — every provider is built against real, documented endpoints and fails closed with `.notConfigured` until real `clientID`/`redirectURI` values are supplied. Two things worth a deliberate decision before real credentials land:
- **"Unsync" (remove local copy, keep cloud)** from the requirements list was not implemented — only "Delete from cloud" exists for the three API providers. The metadata-only architecture (no local file download for dedup) makes "unsync" a separate, not-yet-designed feature; flagging as a real scope cut, not an oversight.
- **Token refresh** is implemented for all three providers (checks `OAuthToken.isExpired`, exchanges the refresh token before use) but has never been exercised against a real expired token, since there's no real token to expire yet.
- One test run (out of ~35 total across this phase's own review cycle) hit an unreproduced flaky failure with no captured diagnostic detail; every specific concurrency race identified by design review (shared `StubURLProtocol` state, shared Keychain provider key, iCloud's real-path TCC permission wall) was fixed and verified stable across many subsequent runs. Noting honestly rather than claiming zero flakiness.
