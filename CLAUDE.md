...
Tokens (`Theme`: colors/gradients/spacing/radii/elevation/module hues/text sizes) and shared components (`BadgeView`, `.careCard()`, `.elevation(_:)`, `.auroraBloom()`, `HeroPanelView`, `HealthDialView`, `ModuleCardView`, `ProgressBarView`, `EmptyStateView`, `StepRowView`, `StickyFooterView`, `SummaryCardView`, sidebar nav-row styles) were transcribed from the Open Design mockups (see below) and are reused across every feature view. Build new UI out of these instead of hand-rolling new card/badge/progress styling. Every module owns a hue via `Theme.hue(for: SidebarDestination)` — tint that module's icon tiles, hero eyebrow, and empty-state illustration with it rather than the generic `Theme.accent`.
...
The UI design lives in the Open Design MCP project **`macos-care-suite`**, currently the **"Aurora Care"** system — an in-place evolution of the prior "Neutral Modern" tokens (same accent/severity/spacing) extended with an `accent-2` gradient partner, an 11-color module hue map, an aurora bloom behind every content area, softer/larger geometry, and two-layer elevation. `styles.css`'s own header comment documents this lineage. Fetch a screen with `mcp__open-design__get_file(project: "macos-care-suite", path: "<screen>.html")` — e.g. `index.html` (dashboard), `junk-scanner.html`, `cloud-cleanup.html`, `performance.html` — and shared tokens from `styles.css`. `onboarding.html`/`paywall.html` intentionally use a separate `atelier.css` editorial system as a deliberate tonal contrast against the rest — don't unify those with the Aurora Care screens. See `plans/0725-1315-apply-open-design-system/` for the original Neutral Modern port and `openspec/changes/adopt-aurora-care-design-system/` (or its archive) for the Aurora Care re-skin. Real screens intentionally omit mockup elements with no backing data (e.g. Dashboard has no health dial — there's no health-scoring module; several screens skip fabricated "last scan" stats) — don't add fake numbers to close the gap; see each view's own header comment for its documented deviations. Settings has no screen yet (`SidebarDestination` has no `.settings` case) — its module hue is defined in `Theme.ModuleHue` but unused until that feature ships.
